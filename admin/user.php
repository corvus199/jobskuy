<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			if($q){ alert("Menghapus Akun User Berhasil !"); redir("user.php"); }
		}  
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			$q = mysql_query("insert into user Values(NULL,'$nama','$email','$telephone','$alamat','$password','$status','$skill')");
			if($q){ alert("Menambah Akun User Berhasil"); redir("user.php"); }
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			if($q){ alert("Mengedit Akun User Berhasil !"); redir("user.php"); }
		}
	}
